...
X_test1 = Input_val[10000:10400,:,:] #For plotting
...
model = load_model('GRU200200NetworkAll240hW.h5')
...
for x in location_set:
    ...
    MAELoc[id] = mean_absolute_error(Y_test[:,id], test_predict[:,id])
    RMSELoc[id] = np.sqrt(mean_squared_error(Y_test[:,id], test_predict[:,id]))
    print(x)
